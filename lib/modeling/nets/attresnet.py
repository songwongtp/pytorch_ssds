...
class _attbasicblock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, expansion=1, downsample=None):
        ...
        self.fc1 = nn.Conv2d(planes * expansion, int(planes * expansion//16), kernel_size=1)  # Use nn.Conv2d instead of nn.Linear
        self.fc2 = nn.Conv2d(int(planes * expansion//16), planes * expansion, kernel_size=1)
        self.sconv = nn.Conv2d(planes * expansion, planes * expansion, kernel_size=3, stride=1, padding=1, groups=planes * expansion, bias=False)
...
def attresnet(conv_defs, depth_multiplier=1.0, min_depth=8):
    ...
    for conv_def in conv_defs:
        ...
        if isinstance(conv_def, BasicBlock):
          for n in range(conv_def.num):
            ...
            layers += [_attbasicblock(in_channels, depth(conv_def.depth), stride, conv_def.t, downsample)]
            in_channels = depth(conv_def.depth * conv_def.t)
        elif isinstance(conv_def, Bottleneck):
          for n in range(conv_def.num):
            (stride, downsample) = (conv_def.stride, _downsample) if n == 0 else (1, None)
            layers += [_attbottleneck(in_channels, depth(conv_def.depth), stride, conv_def.t, downsample)]
            in_channels = depth(conv_def.depth * conv_def.t)
    return layers
...
attresnet_18 = wrapped_partial(attresnet, conv_defs=V18_CONV_DEFS, depth_multiplier=1.0)
attresnet_34 = wrapped_partial(attresnet, conv_defs=V34_CONV_DEFS, depth_multiplier=1.0)

attresnet_50 = wrapped_partial(attresnet, conv_defs=V50_CONV_DEFS, depth_multiplier=1.0)
attresnet_101 = wrapped_partial(attresnet, conv_defs=V101_CONV_DEFS, depth_multiplier=1.0)
