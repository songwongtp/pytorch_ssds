...
class AttFSSD(nn.Module):
    ...
    def __init__(self, base, extras, head, features, attentions, feature_layer, num_classes):
        super(AttFSSD, self).__init__()
    ...
    def forward(self, x, phase='eval'):
        ...
        x = self.norm(x)
        for k, (v, a) in enumerate(zip(self.pyramids, self.attentions)):
            x = v(x)
            x = a(x)
            pyramids.append(x)
...
def add_extras(base, feature_layer, mbox, num_classes):
    ...
    for layer, depth, box in zip(feature_layer[1][0], feature_layer[1][1], mbox):
        ...
        attention_layers += [AttentionBlock(in_channels)]
    ...
    return base, extra_layers, (feature_transform_layers, pyramid_feature_layers), attention_layers, (loc_layers, conf_layers)

def build_attfssd(base, feature_layer, mbox, num_classes):
    base_, extras_, features_, attentions_, head_ = add_extras(base(), feature_layer, mbox, num_classes)
    return AttFSSD(base_, extras_, head_, features_, attentions_, feature_layer, num_classes)
