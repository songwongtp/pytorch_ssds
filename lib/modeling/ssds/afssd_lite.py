...
class AFSSDLite(nn.Module):
    ...
    def __init__(self, base, extras, head, features, feature_layer, num_classes):
        ...
        self.attention = features[1]
        self.pyramids = nn.ModuleList(features[2])
        # print(self.base)
...
def add_extras(base, feature_layer, mbox, num_classes):
    ...
    attention_layer = AttentionBlock(in_channels)

    for layer, depth, box in zip(feature_layer[1][0], feature_layer[1][1], mbox):
        if layer == 'S':
            pyramid_feature_layers += [_conv_dw(in_channels, depth, stride=2, padding=1, expand_ratio=1)]
            in_channels = depth
        elif layer == '':
            pad = (0,1)[len(pyramid_feature_layers)==0]
            pyramid_feature_layers += [_conv_dw(in_channels, depth, stride=1, padding=pad, expand_ratio=1)]
            in_channels = depth
        else:
    ...
    return base, extra_layers, (feature_transform_layers, attention_layer, pyramid_feature_layers), (loc_layers, conf_layers)

def build_afssd_lite(base, feature_layer, mbox, num_classes):
    base_, extras_, features_, head_ = add_extras(base(), feature_layer, mbox, num_classes)
    return AFSSDLite(base_, extras_, head_, features_, feature_layer, num_classes)
